...
// it here, and we assert it returns to its BASELINE size so a leaked entry
// fails loudly here rather than corrupting an unrelated suite. Baseline is
// captured once at module load (not hardcoded to 0) because real per-species
// shiny effects now register permanently via the shinyEffects/ barrel
// (imported once, by combatEngine.ts) as each trait-group batch lands —
// this file's own tests never register 'tangela' as their real target, so
// the baseline is stable across every test in this file regardless of how
// many species are shipped.
