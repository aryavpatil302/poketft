...
// One "side" (shiny or non-shiny) of a ShinyStageRow — same names, same order,
// same meaning as UnitAggregate.perTier's per-star-level entry, so the two read
// as one vocabulary and are directly comparable.
export interface ShinyStageSide {
  fields: number; winRate: number
  avgDealt: number; avgTaken: number; avgCasts: number; avgKills: number; avgDeaths: number
  avgHealSelf: number; avgHealAlly: number; avgShieldSelf: number; avgShieldAlly: number
}
// Per species per stage: the shiny side and that same species' non-shiny side
// at the same stage, so a reader never has to cross-reference the `units`
// array. `nonShiny` is null when that species never fought non-shiny at that
// stage (rendered as an explicit no-comparison marker, not a missing row).
export interface ShinyStageRow { defId: string; stage: number; shiny: ShinyStageSide; nonShiny: ShinyStageSide | null }
...
export interface LeagueReport {
  ...
  // Shiny-Pokémon observability (Tier 1, this run only): win rate for whether
  // the board fielded ANY shiny, and per-shiny-species win rate, both by stage.
  shinyPresence: LearnedAffinityRow[]   // stage|('shiny'|'none')
  shinySpecies: LearnedAffinityRow[]    // stage|definitionId
  // Shiny-effect impact (Tier 2, this run): per-species rollup of how much combat
  // bonus each shiny's ability/effect actually contributed, summed across every
  // traced fight — from the "shiny:<defId>" keys credited into traitDmg/Heal/
  // Shield/Mitigated. `fights` = unit-fights where that shiny contributed anything.
  shinyImpact: { defId: string; dmg: number; heal: number; shield: number; mitigated: number; fights: number }[]
  // Stage-broken-down shiny-vs-non-shiny performance (this run): per species per
  // stage, the shiny side's win rate / avg damage / avg heal / avg shield etc.,
  // and that same species' non-shiny side at the same stage — for direct
  // comparison. This is ADDITIVE to shinyPresence/shinySpecies/shinyImpact, not
  // a replacement: shinyPresence only says whether fielding ANY shiny correlated
  // with winning; shinySpecies gives win rate only, no other stats; shinyImpact
  // is the shiny effect's own bonus magnitude, not stage-scoped and with no
  // non-shiny baseline to compare against. None of the three can answer "how did
  // shiny Snorunt's win rate / damage / healing / shielding at stage 4 compare
  // to non-shiny Snorunt at stage 4" — this field can.
  shinyStageStats: ShinyStageRow[]
...
// A "shiny:<definitionId>" key is a shiny Pokémon's combat effect crediting its
// contribution into the same trait tallies (see src/core/systems/shinyEffects/
// and the isShiny branches in src/core/abilities/) — render it as "✦ <Species>".
const traitName  = (id: string): string =>
  id.startsWith('shiny:')
    ? '✦ ' + (UNIT_MAP.get(id.slice(6))?.name ?? id.slice(6))
    : (TRAIT_MAP.get(id)?.name ?? id)
...
export function renderReportHtml(r: LeagueReport): string {
  ...
  const learnedShinyPresenceRows = r.shinyPresence.slice(0, 30).map(row => {
    const [stage, label] = row.key.split('|')
    return [stage, label === 'shiny' ? 'Shiny fielded' : 'No shiny', pct(row.winRate), String(row.samples)]
  })
  const learnedShinySpeciesRows = r.shinySpecies.slice(0, 30).map(row => {
    const [stage, defId] = row.key.split('|')
    return [stage, unitLink(defId, esc(unitName(defId))), pct(row.winRate), String(row.samples)]
  })
  const hasLearnedData = r.traitPairs.length || r.unitContexts.length || r.traitDepths.length || r.breadths.length
    || r.shinyPresence.length || r.shinySpecies.length
  const learnedSection = hasLearnedData ? `
    ...
    ${r.shinyPresence.length ? `<h3>Fielding a shiny Pokémon</h3>${table(['Stage', 'Board', 'Win rate', 'Samples'], learnedShinyPresenceRows, [false, false, true, true])}` : ''}
    ${r.shinySpecies.length ? `<h3>Which shiny species performed</h3>${table(['Stage', 'Shiny', 'Win rate', 'Samples'], learnedShinySpeciesRows, [false, false, true, true])}` : ''}
  ` : ''

  // Shiny effect impact (Tier 2) — per-species rollup of shiny buff magnitude.
  const shinyNum = (v: number): string => v >= 1 ? String(Math.round(v)) : '<span class="mut">—</span>'
  const shinyImpactSection = (r.shinyImpact ?? []).length ? `
    <h2>Shiny effect impact</h2>
    <p class="sub">Per-species total of the combat bonus each shiny's ability/effect actually contributed, summed across every traced fight (from the <code>shiny:</code> keys credited into the trait tallies — see src/core/systems/shinyEffects/ and the isShiny branches in src/core/abilities/). Shiny-scaled abilities credit the marginal bonus; separate shiny-only effects and shiny-scaled shields/heals credit their full amount. <b>Mitigated</b> = incoming damage a shiny-granted shield absorbed.</p>
    ${table(['Shiny', 'Bonus dmg', 'Healing', 'Shielding', 'Mitigated', 'Fights'],
      r.shinyImpact.map(s => [
        `<span class="tag">${esc(traitName('shiny:' + s.defId))}</span>`,
        shinyNum(s.dmg), shinyNum(s.heal), shinyNum(s.shield), shinyNum(s.mitigated), String(s.fights),
      ]),
      [false, true, true, true, true, true])}
  ` : ''

  // Shiny-vs-non-shiny stage breakdown — species-filtered side-by-side table.
  // Groups r.shinyStageStats (already sorted defId asc, stage asc) by species;
  // chips order by total shiny fields descending so the best-sampled species
  // leads. Uses the .cardctl/.chip/data-* idiom (see cardControls above).
  const shinyStageBySpecies = new Map<string, typeof r.shinyStageStats>()
  for (const row of r.shinyStageStats) {
    const l = shinyStageBySpecies.get(row.defId) ?? []
    l.push(row)
    shinyStageBySpecies.set(row.defId, l)
  }
  const shinyStageSpeciesOrder = [...shinyStageBySpecies.keys()].sort((x, y) => {
    const tx = shinyStageBySpecies.get(x)!.reduce((s, row) => s + row.shiny.fields, 0)
    const ty = shinyStageBySpecies.get(y)!.reduce((s, row) => s + row.shiny.fields, 0)
    return ty - tx
  })
  const shinyStageChips = shinyStageSpeciesOrder.map((defId, i) =>
    `<button class="chip ss-chip${i === 0 ? ' is-on' : ''}" data-def="${defId}">${spriteImg(defId, 20)} ${esc(unitName(defId))}</button>`).join('')
  // Healing/shielding collapse self+ally into one "self / ally" cell (keeps the
  // table readable at this width).
  const helpCell = (self: number, ally: number): string => `${Math.round(self)} / ${Math.round(ally)}`
  // Signed win-rate delta (shiny vs non-shiny) — the whole point of the section,
  // readable without arithmetic. positive = c-good, negative = c-bad.
  const winDelta = (shinyWr: number, nonShinyWr: number): string => {
    const d = shinyWr - nonShinyWr
    const cls = d > 0 ? 'c-good' : d < 0 ? 'c-bad' : 'c-mid'
    const sign = d > 0 ? '+' : d < 0 ? '−' : '±'
    return ` <span class="${cls}">${sign}${Math.abs(Math.round(d * 100))}%</span>`
  }
  const shinyStageBlocks = shinyStageSpeciesOrder.map((defId, i) => {
    const rows = shinyStageBySpecies.get(defId)!
    const tableRows: string[][] = []
    for (const row of rows) {
      const s = row.shiny, ns = row.nonShiny
      tableRows.push([
        String(row.stage), `<span class="tag">${esc(traitName('shiny:' + defId))}</span>`,
        String(s.fields), `${pct(s.winRate)}${ns ? winDelta(s.winRate, ns.winRate) : ''}`,
        String(Math.round(s.avgDealt)), String(Math.round(s.avgTaken)),
        helpCell(s.avgHealSelf, s.avgHealAlly), helpCell(s.avgShieldSelf, s.avgShieldAlly),
      ])
      if (ns) {
        tableRows.push([
          String(row.stage), 'Ordinary', String(ns.fields), pct(ns.winRate),
          String(Math.round(ns.avgDealt)), String(Math.round(ns.avgTaken)),
          helpCell(ns.avgHealSelf, ns.avgHealAlly), helpCell(ns.avgShieldSelf, ns.avgShieldAlly),
        ])
      } else {
        tableRows.push([String(row.stage), '<span class="mut">no comparison at this stage</span>', '', '', '', '', '', ''])
      }
    }
    return `<div class="ssblock" data-def="${defId}" style="${i === 0 ? '' : 'display:none'}">
      <h3>${unitLink(defId, esc(unitName(defId)))}</h3>
      ${table(['Stage', 'Variant', 'Fielded', 'Win rate', 'Avg dmg', 'Avg taken', 'Avg heal (self/ally)', 'Avg shield (self/ally)'], tableRows,
        [false, false, true, true, true, true, true, true])}
    </div>`
  }).join('')
  const shinyStageSection = r.shinyStageStats.length ? `
    <h2>Shiny vs non-shiny performance by stage</h2>
    <p class="sub">Per-fight averages over fights where that species was fielded, gated at a minimum shiny-side sample count (the shiny row's Fielded column); the ordinary row is ungated context, with its own sample count in its own Fielded column. A draw counts as a loss for both sides, same convention as the rest of the report.</p>
    <div class="cardctl"><div class="cardctl-grp"><span class="cardctl-lbl">Species</span>${shinyStageChips}</div></div>
    ${shinyStageBlocks}
  ` : ''
...
</div>${rptData}${UR_RENDERER}${CARD_CONTROLS}${SHINY_STAGE_CONTROLS}${ROUTER}`
...
// Client control bar for the shiny-vs-non-shiny stage section: click a species
// chip to show only that species' block (data-def match), hide the rest. Same
// defensive, ES5-only IIFE style as CARD_CONTROLS — bails out early if the
// control bar or blocks aren't present (e.g. no shiny ever fielded).
const SHINY_STAGE_CONTROLS = `<script>
(function(){
  var chips = [].slice.call(document.querySelectorAll('.ss-chip'));
  var blocks = [].slice.call(document.querySelectorAll('.ssblock'));
  if(!chips.length || !blocks.length) return;
  function apply(defId){
    chips.forEach(function(c){ c.classList.toggle('is-on', c.dataset.def===defId); });
    blocks.forEach(function(b){ b.style.display = (b.dataset.def===defId) ? '' : 'none'; });
  }
  chips.forEach(function(c){ c.addEventListener('click', function(){ apply(c.dataset.def); }); });
  var onChips = chips.filter(function(c){ return c.classList.contains('is-on'); });
  var initial = onChips.length ? onChips[0] : chips[0];
  if(initial) apply(initial.dataset.def);
})();
</script>`
