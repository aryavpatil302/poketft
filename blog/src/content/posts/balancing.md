...
*The report's landing view, from a 100-game run. This is what I open first after any change, before I go looking at anything more specific.*
...
*One real fight, not an average. The panel on the right is the one I actually care about here: cumulative damage across the whole board, one line per unit. A flat line that suddenly jumps near the end is a burst caster waiting on mana. A steady climb from second one is a marksman just auto-attacking the whole time. The same total damage can hide two completely different combat roles.*
...
*Every unit gets this drill-down: win rate, damage and healing colored against its own archetype average, and a trait-by-trait breakdown of what's actually contributing. I can scan a page like this in about ten seconds and know exactly which numbers earned the green arrow.*
...
*The feed itself. "TangelaTruther" walking back their own jungle take is right there next to a thread that's basically calling Armarouge broken three days before I actually checked. It reads like an actual argument between people who each have half a point, not a verdict.*
...
*A different thread, same shape of argument as the wide-boards one: someone posts a comp that feels unfair, and the replies don't just pile on, they converge on the actual mechanical reason (stacking two trait payoffs on the same casts instead of splitting the board between two plans). A single AI-generated summary would've flattened this into "the comp is strong." The replies are the part worth reading.*
