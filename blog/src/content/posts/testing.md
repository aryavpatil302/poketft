...
*The whole test rig in one screenshot: pick any unit at any star level on the left, drop preset dummies from the panel on the right, and control combat speed from the widget at the bottom. Everything below started as a "let me check something real quick" in this exact view.*
...
*Charizard against three dummies and nothing else. No enemy comp, no randomness, no second unit's damage muddying the read, just one Pokemon's kit measured against a stable, known target.*
